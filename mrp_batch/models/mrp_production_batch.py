...
CONFIG_ENABLE_DELAY_ACTION_ASSIGN = CONFIG_ENABLE_DELAY_ACTION_ASSIGN
CONFIG_ENABLE_DELAY_ACTION_CONFIRM = CONFIG_ENABLE_DELAY_ACTION_CONFIRM
CONFIG_ENABLE_DELAY_ACTION_UNRESERVE = CONFIG_ENABLE_DELAY_ACTION_UNRESERVE
CONFIG_ENABLE_DELAY_ACTION_RESERVE = CONFIG_ENABLE_DELAY_ACTION_RESERVE
CONFIG_ENABLE_DELAY_ACTION_DONE = CONFIG_ENABLE_DELAY_ACTION_DONE
CONFIG_ENABLE_DELAY_ACTION_CANCEL = CONFIG_ENABLE_DELAY_ACTION_CANCEL
CONFIG_ENABLE_DELAY_COMPONENT_AVAILABILITY = CONFIG_ENABLE_DELAY_COMPONENT_AVAILABILITY
CONFIG_ENABLE_DELAY_COMPONENT_AVAILABILITY_DETAILS = CONFIG_ENABLE_DELAY_COMPONENT_AVAILABILITY_DETAILS
...
class MrpProductionBatch(models.Model):
    ...
    # COLUMNS
    def _get_bool_param(self, param_value):
        """Helper method to convert config parameter string to boolean."""
    ...
    def _compute_duration_by_type(self, productions, workorders, duration_field, operation_type=None):
        """
        Helper method to compute duration by operation type.
        Reduces code duplication across multiple duration computation methods.
        
        :param productions: Filtered production records
        :param workorders: Workorder records
        :param duration_field: Field name to use ('duration' or 'duration_expected')
        :param operation_type: Optional operation type filter ('build', 'test', 'other')
        :return: Sum of durations
        """
        if operation_type:
            filtered_wos = workorders.filtered(lambda wo: wo.operation_type == operation_type)
            return sum(getattr(wo, duration_field, 0.0) for wo in filtered_wos)
        else:
            # Total duration across all workorders
            return sum(getattr(wo, duration_field, 0.0) for wo in workorders)

    def _compute_avg_duration_by_type(self, workorders, duration_field, operation_type):
        """
        Helper method to compute average duration by operation type.
        
        :param workorders: Workorder records
        :param duration_field: Field name to use ('duration' or 'duration_expected')
        :param operation_type: Operation type filter ('build', 'test', 'other')
        :return: Average duration
        """
        filtered_wos = workorders.filtered(lambda wo: wo.operation_type == operation_type)
        if not filtered_wos:
            return 0.0
        durations = [getattr(wo, duration_field, 0.0) for wo in filtered_wos]
...
#########
    ...
    def action_assign(self):
        ...
        for rec in self:
            ...
            enable_delay = (
                ...
                .sudo()
                .get_param(CONFIG_ENABLE_DELAY_ACTION_ASSIGN)
            )
            for mo in rec.production_ids.filtered(
                lambda x: x.state not in (STATE_DRAFT, STATE_DONE, STATE_CANCEL)
            ):
                if self._get_bool_param(enable_delay):
    ...
    def action_confirm(self):
        ...
        for rec in self:
            ...
            enable_delay = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param(CONFIG_ENABLE_DELAY_ACTION_CONFIRM)
            )
            for mo in rec.production_ids.filtered(lambda x: x.state == STATE_DRAFT):
                if self._get_bool_param(enable_delay):
    ...
    def button_plan(self):
        for rec in self:
            ...
            if productions:
                ...
                if rec.state == STATE_DRAFT and not any(
                    productions.filtered(lambda p: p.state == STATE_DRAFT)
                ):
                    rec.state = "confirm"
            if queued:
                rec.is_queuing = True
    ...
    def action_unreserve(self):
        for rec in self:
            ...
            enable_delay = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param(CONFIG_ENABLE_DELAY_ACTION_UNRESERVE)
            )
    ...
    def action_done(self):
        for rec in self:
            ...
            enable_delay = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param(CONFIG_ENABLE_DELAY_ACTION_DONE)
            )

            for mo in rec.production_ids.filtered(
                lambda x: x.state not in (STATE_DONE, STATE_CANCEL)
            ):
                if self._get_bool_param(enable_delay):
    ...
    def action_cancel(self):
        for rec in self:
            queued = False
            rec.state = STATE_CANCEL
            enable_delay = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param(CONFIG_ENABLE_DELAY_ACTION_CANCEL)
            )

            eligible_productions = rec.production_ids.filtered(
                lambda mo: not any(wo.state == STATE_DONE for wo in mo.workorder_ids)
            )
            for mrp_production in eligible_productions:
                if self._get_bool_param(enable_delay):
    ...
    def _compute_build_test_unit_other_durations(self):
        """Compute real average unit durations by operation type using helper method."""
        for rec in self:
            productions = rec.production_ids.filtered(lambda p: p.state != STATE_CANCEL)
            workorders = productions.mapped("workorder_ids")

            rec.avg_build_duration = self._compute_avg_duration_by_type(
                workorders, "duration", OPERATION_TYPE_BUILD
            )
            rec.avg_test_duration = self._compute_avg_duration_by_type(
                workorders, "duration", OPERATION_TYPE_TEST
            )
            rec.avg_other_duration = self._compute_avg_duration_by_type(
                workorders, "duration", OPERATION_TYPE_OTHER
            )
    ...
    def _compute_revenue(self):
        for rec in self:
            ...
            valid_productions = rec.production_ids.filtered(
                lambda mo: mo.state != STATE_CANCEL
            )
            # Compute revenue
            total = sum(
    ...
    def _compute_mrp_production_cancel(self):
        ...
        for rec in self:
            ...
            if rec.production_ids:
                rec.is_cancel = all(
                    production.state in (STATE_DONE, STATE_CANCEL)
    ...
    def _compute_is_planned(self):
        for rec in self:
            ...
            # Tags will be recomputed automatically via dependencies
        ...
        for rec in self:
            ...
            if rec.production_ids and filtered_productions:
                rec.is_confirmed = not any(
                    production.state == STATE_DRAFT for production in rec.production_ids
                )
    ...
    def _compute_lock(self):
        ...
        for rec in self:
            ...
            if rec.production_ids and rec.state != "cancel":
                rec.show_lock = all(
                    wo.state in (STATE_DONE, STATE_CANCEL)
                    for production in rec.production_ids
    ...
    def _compute_components_availability(self):
        ...
        for batch in self:
            ...
            valid_productions = batch.production_ids.filtered(
                lambda mo: mo.state not in (STATE_DRAFT, STATE_CANCEL, STATE_DONE, STATE_TO_CLOSE)
            )
            ...
            # Fetch all raw moves - ORM will handle forecast_availability via dependencies
            all_raw_moves = valid_productions.move_raw_ids
            ...
            for move in all_raw_moves:
                ...
                required_qty = 0 if move.state == STATE_DRAFT else move.product_qty
                if (
                    float_compare(
            ...
            enable_component_details_delay = (
                self.env["ir.config_parameter"]
                .sudo()
                .get_param(CONFIG_ENABLE_DELAY_COMPONENT_AVAILABILITY_DETAILS)
            )
            if self._get_bool_param(enable_component_details_delay):
