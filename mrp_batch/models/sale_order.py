...
class SaleOrder(models.Model):
    ...
    mrp_batch_count = fields.Integer(string="mrp batch count",compute='_compute_mrp_production_batch_id_count')
    ...
    def split_mo(self):
        ...
        for rec in self:
            ...
            for mo in rec.mrp_production_ids:
                if mo.product_id.tracking == 'serial' and mo.product_id.is_allowe_split_mo:
                    qty = mo.product_qty
                    if qty > 1:
                        mo.sudo()._split_productions({mo: ([1] * int(qty))})[:-1]
            mrp_batch_id = batch_obj.create(vals)
            rec.mrp_production_ids.write({
                'mrp_batch_id' : mrp_batch_id.id 
            })
    ...
    def action_view_mrp_production_batch(self):
        ...
        if len([batch.id for batch in self.mrp_production_ids.mrp_batch_id]) == 1:
            ...
            action.update({
                'name': _("Manufacturing Production Batch Generated by %s", self.name),
