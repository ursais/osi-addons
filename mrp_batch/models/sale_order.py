    ...
    def split_mo(self):
        ...
        for order in self:
            ...
            for mo in order.mrp_production_ids:
                ...
                # Handle MOs with qty <= 1: process them without splitting
                if mo.product_qty <= 1:
                    # Assign batch if batch_mode == "single"
                    if existing_batch:
                        mo.sudo().write({"mrp_batch_id": existing_batch.id})
                    elif not mo.mrp_batch_id:
                        mo.write(
                            {
                                "mrp_batch_id": batch_obj.create(
                    ...
                    # Ensure moves belong to the procurement group
                    ...
                    # Update move quantities to match MO qty
                    for move in mo.move_finished_ids:
                        move.product_uom_qty = mo.product_qty
