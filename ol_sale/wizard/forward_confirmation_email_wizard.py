...
class ForwardConfirmationEmailWizard(models.TransientModel):
    ...
    text_intro = fields.Text(
        string='Introduction text', help='This will get inserted to the start of the email'
    )
    text_body = fields.Text(string='Body text', help='This will get inserted before the product list')
    text_closing = fields.Text(string='Closing text', help='This will get inserted to end of the email')
    ...
    def forward_confirmation_email(self):
        ...
        ).send_email_with_terms_and_conditions(self.order_id.company_id, self.order_id.id)
