        ...
        for rec in self:
            ...
            if rec.production_ids and rec.production_ids.sale_order_id:
                ...
                rec.account_manager_id = (
                    rec.production_ids.sale_order_id.account_manager_id.id
                )
    ...
    def _compute_date_start(self):
        for record in self:
            for production in record.production_ids.filtered("sale_order_id"):
                record.date_start = production.sale_order_id.mapped("commitment_date")[
                    0
                ]
    ...
    def _compute_allocation_date(self):
        for batch in self:
            for production in batch.production_ids:
                ...
                if (
                    batch.date_start
                    and production.picking_type_id.reservation_method == "by_date"
                ):
                    allocation_date = batch.date_start - timedelta(
                        days=production.picking_type_id.reservation_days_before
                    )
