...
class TestOlPurchaseLastPriceInfo(common.TransactionCase):
    def setUp(self):
        super().setUp()
    ...
    def test_ol_purchase_last_price_info_demo(self):
        ...
        purchase_lines = self.purchase_line_model.search(
            [
                ...
                ("partner_id.company_id", "not in", self.env.companies.ids),
            ]
    ...
    def test_ol_purchase_last_price_info_new_order(self):
        ...
        if (first_order_line.partner_id.company_id.id not in self.env.companies.ids):
            ...
            self.assertNotEqual(first_order_line.price_unit, self.product.last_purchase_price)
            self.assertNotEqual(
                first_order_line.currency_id,
                self.product.last_purchase_currency_id,
            )
            self.assertNotEqual(self.product.last_purchase_currency_id, self.currency_extra)
            self.assertNotEqual(self.product.last_purchase_price_currency, 2.0)
            self.assertNotEqual(self.partner, self.product.last_purchase_supplier_id)
