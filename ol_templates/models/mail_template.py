...
class MailTemplate(models.Model):
    ...
    def send_email_with_terms_and_conditions(
        self, company, res_id, force_send=False, raise_exception=False, email_values=None, notif_layout=False
    ):
        """
        Find and set the T&C attachments for the template
        Replicates the core `send_email()` function signature and calls it with updated attachments
        """
        if not company:
            # If by some error the SO did not have a company set on it we want to set a default value.
        ...
        # Override the attachements by adding the Ts & Cs attachment to the existing ones
        if not email_values:
            email_values = {}
