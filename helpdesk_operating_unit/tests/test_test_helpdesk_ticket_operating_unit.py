...
class TestHelpdeskTicketOperatingUnit(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.helpdesk_ticket1 = self._create_helpdesk_ticket(self.user1.id,
                                                             self.main_OU)
        self.helpdesk_ticket2 = self._create_helpdesk_ticket(self.user2.id,
                                                             self.b2c_OU)
    ...
    def test_helpdesk(self):
        ...
        self.assertEqual(helpdesk_ticket_ids.ids, [], 'User 2 should not have '
                         ' access to %s' % self.main_OU.name)
        self.assertEqual(self.helpdesk_ticket1.operating_unit_id.id,
                         self.main_OU.id)
