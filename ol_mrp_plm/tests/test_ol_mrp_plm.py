...
class TestPLMProductState(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.ecostage_progress.write({"product_state_id": cls.product_state_sellable.id, "allow_bom_edits": False})
    ...
    def test_product_allow_modification(self):
        "The invoice_policy field of a product can only be modified by an admin user."
        with self.assertRaises(ValidationError):
            self.product.with_user(self.test_user_demo).write({"invoice_policy": "delivery"})
        self.assertNotEqual(self.product.invoice_policy, "delivery")

        self.product.with_user(self.test_user_admin).write({"invoice_policy": "delivery"})
        self.assertEqual(self.product.invoice_policy, "delivery")

    def test_bom_allow_modification(self):
        "The product_qty field of a product can only be modified by an admin user."
        self.assertEqual(self.bom_1.product_id.product_state_id, self.product_state_draft)
        with self.assertRaises(ValidationError):
    ...
    def test_change_product_stages(self):
        "The product_state_id of a product can be changed by writing to the stage_id field of ECO."
        mrp_eco = self.env["mrp.eco"].create(
            [
                {
                    "name": "Test MRP ECO",
        ...
        self.assertNotEqual(self.product.product_state_id, self.ecostage_progress.product_state_id)
        mrp_eco.write({"stage_id": self.ecostage_progress.id})
        self.assertEqual(self.product.product_state_id, self.ecostage_progress.product_state_id)
