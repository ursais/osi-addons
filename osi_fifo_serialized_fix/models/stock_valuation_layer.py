...
class StockValuationLayer(models.Model):
    ...
    def create(self, vals):
        ...
        for layer in res.filtered(
            ...
            # Below code is only for Cabinotch as they are using 2-step manufacturing
            if layer.stock_move_id.mapped("move_dest_ids").mapped(
            ...
            if layer.stock_move_id.raw_material_production_id:
                ...
                for lot in layer.lot_ids:
                    ...
                    # all_moves = (
                    #     all_lot_layers.mapped("stock_move_id")
                    #     or self.env["stock.move"]
                    # )
                    # rawmaterial_moves = all_lot_layers.mapped(
                    #     "stock_move_id"
                    # ).mapped("move_dest_ids").filtered(
                    #     lambda l : l.raw_material_production_id
                    # ) or self.env["stock.move"]
                    # required_moves = all_moves - rawmaterial_moves
                    # required_layers = self.search(
                    #     [('stock_move_id','in',required_moves.ids)]
                    # )
                    total_qty = sum([svl.quantity for svl in all_lot_layers])
                    total_value = sum([svl.value for svl in all_lot_layers])
                    svl_value = total_value / total_qty if total_qty else 1.0
            ...
            #     consumed_moves = production.move_raw_ids.filtered(
            ...
            #             sum(
            #                 -m.stock_valuation_layer_ids.value
            #                 for m in consumed_moves.filtered(
            #                     lambda sm: sm.product_id.tracking == "serial"
            #                 )
            #             )
            #         )
            ...
            #         )
            #         raw_sn_price = sum(
            #             lot.real_price for lot in consumed_moves.mapped("lot_ids")
            #         )
            #         raw_non_sn_price = -sum(
            #             consumed_moves.filtered(
            ...
            #             lambda m: m.state in ("done", "cancel") and m.quantity_done > 0
            #         )
            ...
            #             if (
            #                 byproduct.product_id.cost_method in ("fifo", "average")
            #                 and byproduct.product_id.tracking == "serial"
            #             ):
            ...
            #                     )
            #                 )
            #                 byproduct.stock_valuation_layer_ids.write(
            #                     {
            #                         "unit_cost": byproduct.price_unit,
            #                         "value": byproduct.price_unit
            #                         * byproduct.product_uom._compute_quantity(
            #                             byproduct.quantity_done,
            #                             byproduct.product_id.uom_id,
            #                         ),
            #                         "remaining_value": byproduct.price_unit
            #                         * byproduct.product_uom._compute_quantity(
            #                             byproduct.quantity_done,
            #                             byproduct.product_id.uom_id,
            #                         ),
            ...
            #                     total_cost
            #                     * float_round(
            #                         1 - byproduct_cost_share / 100,
            #                         precision_rounding=0.0001,
            #                     )
            #                     / qty_done
            #                 )
            #                 production.lot_producing_id.real_price = (
            #                     finished_move.price_unit
            #                 )
