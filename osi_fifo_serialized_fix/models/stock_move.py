                ...
                if ji_ids:
                    if len(svl_ids) > 1:
                        ...
                        for ji_id in ji_ids:
                            if ji_id.credit != 0:
                                ji_id.with_context(check_move_validity=False).\
                                    write({'credit': svl.unit_cost * move.stock_valuation_layer_ids.stock_move_id.product_uom_qty})
                            else:
                                ji_id.with_context(check_move_validity=False).\
                                    write({'debit': svl.unit_cost * move.stock_valuation_layer_ids.stock_move_id.product_uom_qty})
                        ji_ids[0].move_id.action_post()
        return res
