...
class SaleOrder(models.Model):
    ...
    @api.depends(
            'tax_on_shipping_address', 'partner_id', 'partner_shipping_id')
    ...
    def compute_tax(self):
        """
        Create and update tax amount for each and every order line
        and shipping line.
        @param order_line: send sub_total of each line and get tax amount
        @param shiping_line: send shipping amount of each ship line
        and get ship tax amount
        """
        ...
        # ship from Address / Origin Address either warehouse
        # or company if none
        ship_from_address_id = (
            self.warehouse_id.partner_id
            ...
            if lines:
                if avatax_config.on_line:
                    ...
                    for line in lines:
                        tax_id = (
                            line['tax_id']
                            and [tax.id for tax in line['tax_id']]
                            or [])
                    ...
                    raise UserError(_(
                        'Please select system calls '
                        'in Avatax API Configuration'))
            else:
                for o_line in self.order_line:
                    o_line.write({'tax_amt': 0.0})
