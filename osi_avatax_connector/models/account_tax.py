...
class AccountTax(models.Model):
    ...
    @api.model
    def _get_compute_tax(self, avatax_config, doc_date, doc_code, doc_type,
                         partner, ship_from_address, shipping_address, lines,
                         user=None, exemption_number=None,
                         exemption_code_name=None, commit=False,
                         invoice_date=False, reference_code=False,
                         location_code=False, is_override=False,
                         currency_id=False):
        ...
        if not avatax_config.address_validation:
            if avatax_config.force_address_validation:
                if not shipping_address.date_validation:
                    raise UserError(_(
                        'Please validate the shipping address for the '
        ...
        origin = BaseAddress(
            ...
            ship_from_address.state_id and
            ship_from_address.state_id.code or None,
            ship_from_address.country_id and
        ...
        destination = BaseAddress(
            ...
            shipping_address.state_id and
            shipping_address.state_id.code or None,
            shipping_address.country_id and
        ...
        result = avalara_obj.get_tax(
            ...
            lines, exemption_number, exemption_code_name,
    ...
    def cancel_tax(self, avatax_config, doc_code, doc_type, cancel_code):
        """Sometimes we have not need to tax calculation, then method is
        used to cancel taxation"""
        if avatax_config.disable_tax_calculation:
            _logger.info(
                'Avatax tax calculation is disabled. Skipping %s %s.',
                doc_code, doc_type)
