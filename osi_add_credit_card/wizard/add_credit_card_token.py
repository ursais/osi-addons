...
class AddCreditCardToken(models.TransientModel):
    ...
    def add_cc_token(self):
        ...
        if not authorize:
            raise UserError(
                    "Error: Authorize.net Needs configuration!"
                )
        if not self.partner_id.zip or not self.partner_id.street or \
            ...
            raise UserError(
                    "Address Validation: Please verify partner address (street, city, zip, state, country)!"
                )
        ...
        payment_token_data = {
            ...
            'cc_number': self.cc_number,
            'cc_expiry': "%s / %s"% (self.cc_expiry_month, self.cc_expiry_year),
            'cc_brand': 'visa',
            'cc_cvc': self.cc_cvc,
            'cc_holder_name': self.partner_id.name,
            }
        # create payment token
        payment_token = self.env['payment.token'].create(payment_token_data)
