...
class Partner(OnLogicBaseObjectType, PartnerInterface):
    """
    ...
    parent_id = graphene.Field(lambda: Partner, required=False)
    create_id = graphene.Field(lambda: Partner)
    update_id = graphene.Field(lambda: Partner)

    @staticmethod
    def resolve_account_manager(partner, _):
        return partner.account_manager.partner_id if partner.account_manager else None
    ...
    def resolve_tax_exemption_code(partner, _):
        ...
        (see set_partner_exemption_data in ls_addons13/ls_account_avatax/models/sale_order.py)
        """
        company_values = Partner.get_company_dependent_field_value(
            base_record=partner,
            field_name=False,
