    ...
    def _compute_domain(self, model_name, mode="read"):
        ...
        if self.env.user.id and read_value and not all_data:
            if model_name not in model_list:
                ...
                # a = self._cr.fetchall()
                a = (
                    self.env["access.management"]
                    .sudo()
                    .browse(row[0] for row in self._cr.fetchall())
                )
                if a:
                    a -= a.filtered(
                        lambda x: x.is_apply_on_without_company == False
                        and self.env.company.id not in x.company_ids.ids
                    )
                    if bool(a):
                        if mode != "read" and model_name not in [
        ...
        if not value:
            ...
            if model_name and model_name in self.env and value == "installed":
                self._cr.execute(
                    SQL("SELECT id FROM ir_model WHERE model='%s'" % model_name)
                )
