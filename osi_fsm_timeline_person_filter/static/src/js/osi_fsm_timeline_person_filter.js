odoo.define('osi_fsm_timeline_person_filter.osi_fsm_timeline_person_filter', function (require) {
    "use strict";
            ...
            if(clear){
                ...
                if(domain){
                    this._rpc({
                        ...
                        var ids  = user_ids;
                        ids.push(false);
                        self.user_domains = ["fsm_person_id","in",ids];
                        var temp_domain = [];
                        if(self.last_domains){
                            temp_domain = _.clone(self.last_domains);
                        }
                        temp_domain.push(self.user_domains);
                        self.do_search_related_user_filter(temp_domain, self.last_contexts, self.last_group_bys,list_user_ids);
                    });
                }
            }
        },
