...
class SaleOrderLine(models.Model):
    ...
    def _compute_product_qty_to_sell(self):
        ...
        for rec in self:
            ...
            context.update({
                ...
                'location': rec.order_id.warehouse_id.lot_stock_id.id})
            # Get Availability of product
            res = rec.product_id.with_context(**context)._product_available()
            # Quantity on hand
            qty_available = res[rec.product_id.id]['qty_available'] if \
                rec.product_id.id in res else 0
            # Get Reserved Qty
            reserved_qty = res[rec.product_id.id]['outgoing_qty'] if \
                rec.product_id.id in res else 0
    ...
    qty_to_sell = fields.Float(
        ...
        compute='_compute_product_qty_to_sell'
    )
