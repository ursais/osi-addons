...
class CCRecStatementLine(models.Model):
    ...
    name = fields.Char(
        "Name", required=True, help="Derived from the related Journal Item."
    )
    ref = fields.Char("Reference", help="Derived from the related Journal Item.")
    partner_id = fields.Many2one(
        "res.partner", string="Partner", help="Derived from the related Journal Item"
    )
    amount = fields.Float(
        string="Amount",
        digits="Account",
    ...
    statement_id = fields.Many2one(
        "cc.rec.statement", string="Statement", required=True, ondelete="cascade"
    )
    move_line_id = fields.Many2one(
        "account.move.line", "Journal Item", help="Related Journal Item."
    )
    cleared_cc_account = fields.Boolean(
        "CC Cleared? ", help="Check if the transaction has cleared from the cc"
    )
    research_required = fields.Boolean(
        "Research Required?",
        help="Check if the transaction should be researched by the accounting " "team.",
    )
        ...
        for rec in self:
            ...
            if rec.move_line_id:
                rec.move_line_id.write(
                    {"cleared_cc_account": False, "cc_rec_statement_id": False}
                )
