...
class CcRecStatement(models.Model):
    ...
    def check_group(self):
        ...
        if group_verifier_id:
            ...
            for statement in self:
                if statement.state != "draft" and self._uid not in group_user_ids:
                    raise UserError(
                        _(
                            "Only a member of '%s' group can delete/edit credit"
    ...
    def copy(self, default=None):
        ...
        default.update(
            {"credit_move_line_ids": [], "debit_move_line_ids": [], "name": ""}
        )
    ...
    def write(self, vals):
        for rec in self:
            ...
            if "journal_id" in vals and vals.get("journal_id"):
                ...
                if (
                    journal
                    and journal.partner_id
                    and journal.partner_id.property_account_payable_id
                ):
                    vals.update(
                        {
                            "account_id": journal.partner_id.property_account_payable_id.id
                        }
                    )
                ...
                line.move_line_id.write(
                    {
                        ...
                        "cc_rec_statement_id": line.cleared_cc_account
                        and rec.id
                        or False,
                    }
                )
    ...
    def action_cancel_draft(self):
        ...
        for rec in self:
            ...
            aml.write({"cleared_cc_account": False, "cc_rec_statement_id": False})
            # Reset 'Cleared' in statement lines
            sline.write({"cleared_cc_account": False, "research_required": False})
            # Reset statement
            rec.write(
    ...
    def refresh_record(self):
            ...
            if rec.state == "draft":
                for cr_item in rec.credit_move_line_ids:
                    if cr_item.move_line_id and cr_item.cleared_cc_account:
                        refdict[cr_item.move_line_id.id] = cr_item.cleared_cc_account

                for dr_item in rec.debit_move_line_ids:
                    if dr_item.move_line_id and dr_item.cleared_cc_account:
                        refdict[dr_item.move_line_id.id] = dr_item.cleared_cc_account
        ...
        for rec in self:
            ...
            if rec.state == "draft":
                ...
                vals = self.fetch_data(ending_date, suppress_ending_date_filter)
    ...
    def get_starting_balance(self, journal_id, account_id, ending_date):
        ...
        for statement in statement_ids:
            if statement.ending_date < ending_date:
                reslist.append((statement.ending_date, abs(statement.ending_balance)))
    ...
    def onchange_suppress_date(self):
        return self.fetch_data(self.ending_date, self.suppress_ending_date_filter)
    ...
    sum_of_credits_lines = fields.Float(
        compute="_compute_balance",
        string="Number of cleared charges",
    ...
    sum_of_ucredits_lines = fields.Float(
        compute="_compute_balance",
        string="Number of uncleared charges",
        help="Credit Card Charges, Fees, Interest and Penalties # of Items",
    )
        ...
        for statement in self:
            ...
            for line in statement.credit_move_line_ids:
                statement.sum_of_credits += (
                    ...
                    and round(line.amount, account_precision)
                    or 0.0
                )
                statement.sum_of_credits_cur += (
                    line.cleared_cc_account
                    and round(line.amountcur, account_precision)
                    or 0.0
                )
                statement.sum_of_credits_lines += line.cleared_cc_account and 1.0 or 0.0
                statement.sum_of_ucredits += (
                    (not line.cleared_cc_account)
                    and round(line.amount, account_precision)
                    or 0.0
                )
            ...
            for line in statement.debit_move_line_ids:
                ...
                statement.sum_of_debits_lines += line.cleared_cc_account and 1.0 or 0.0
            ...
            statement.difference = round(
                (
                    abs(statement.starting_balance - statement.ending_balance)
                    - statement.cleared_balance
                ),
                account_precision,
            )
            statement.difference_cur = round(
                (
                    abs(statement.starting_balance - statement.ending_balance)
