...
class PurchaseOrderLine(models.Model):
    ...
    def onchange_product_id(self):
        if self.product_id:
            ...
            self.calculate_from_date = self.product_id.calculate_from_date or\
    ...
    def create(self, vals):
        ...
        for product in self.env['product.product'].browse(vals['product_id']):
            ...
            vals['calculate_from_date'] = product.calculate_from_date or\
        ...
        if 'product_id' in vals:
            for product in self.env['product.product'].browse(
                ...
                vals.update({
                    ...
                    'calculate_from_date': product.calculate_from_date or
                    product.product_tmpl_id.
                    calculate_from_date,
        ...
        if 'product_uom' in vals:
            ...
            if self.product_id.uom_id.id != new_uom:
                avg_price = self.product_id.uom_id._compute_price(
                    self.product_id.avg_price,
                    self.env['uom.uom'].browse(new_uom))
