...
class ReportRmaRepairSummary(models.AbstractModel):
    _name = 'report.ls_pdf_reports.report_rma_repair_summary'
    ...
    def get_product_lines(self, rmas):
        ...
        for rma in rmas:
            ...
            rma_lines = rma.used_rma_line_ids.filtered(
                lambda l: l.line_product_type in ['component', 'system', 'phantom'] and l.repair_order_ids
            )
            ...
            for rma_line in rma_lines:
                ...
                if not rma_line.repair_order_ids.filtered(lambda ro: ro.state in ['done']):
                ...
                if rma_line.lot_id:
                    ...
                    if rma_line.line_product_type == 'phantom':
                        ...
                        sale_order_line_ph_kit_calc[rma_line.sale_order_line][rma_line.product_id] += 1
                        current_ph_kit_component_qty = sale_order_line_ph_kit_calc[rma_line.sale_order_line][
                            rma_line.product_id
                        ]

                        # How many of this component we have in the phantom kit
                        component_qty_in_ph_kit = (
                            rma_line.sale_order_line.product_id.phantom_bom_id.bom_line_ids.filtered(
                                lambda l, rma_line=rma_line: l.product_id == rma_line.product_id
                            ).product_qty
                        )
                        ...
                        number_of_ph_kits = ceil(
                            float(current_ph_kit_component_qty) / float(component_qty_in_ph_kit)
                        )
                    ...
                    non_serial_line_list[product]['notes'] += '</br>{}'.format(self.get_notes(rma_line))
    ...
    def get_notes(self, rma_line):
        ...
        for repair_order in rma_line.repair_order_ids:
            ...
            notes += u'{note}'.format(note=repair_order.external_notes.strip())
