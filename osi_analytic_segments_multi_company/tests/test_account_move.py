...
class TestAccountMove(common.TransactionCase):
    def setUp(self):
        ...
        self.account_payroll_clearing_main_company = self.env["account.account"].create(
            {
                "code": "Test 212410",
        ...
        self.account_due_from_main_company = self.env["account.account"].create(
            {
                "code": "Test 100020",
        ...
        user_type_current_expenses = self.env.ref("account.data_account_type_expenses")

        self.account_salary_expense_main_company = self.env["account.account"].create(
            {
                "code": "Test 212400",
        ...
        self.account_invoice_main_company = self.env["account.account"].create(
            {
                "code": "Invoice 062320",
                "name": "Test Invoices",
                "user_type_id": user_type_current_expenses.id,
        ...
        self.account_invoice_company_one = self.env["account.account"].create(
            {
                "code": "Invoice 062320",
                "name": "Test Invoices",
                "user_type_id": user_type_current_expenses.id,
        ...
        self.company_one.due_from_account_id = self.account_due_from_company_one
        ...
        self.partner_company_two = (
            self.env["res.partner"].sudo().create({"name": "Test Company Two"})
        )
        self.company_two = (
            self.env["res.company"]
            .sudo()
        ...
        self.company_two.due_from_account_id = self.account_due_from_company_two
        self.company_two.due_to_account_id = self.account_due_to_company_two

        self.employee_A = self.env["res.partner"].create({"name": "Employee A"})

        self.employee_B = self.env["res.partner"].create({"name": "Employee B"})

        self.employee_C = self.env["res.partner"].create({"name": "Employee C"})
    ...
    def test_distribution_segments(self):
        ...
        self.due_to = (
            ...
            .search(
                [
                    ("company_id", "=", self.company_one.id),
                    ("ref", "like", invoice_id.name),
                ]
            )
        )

        self.assertEqual(
            self.due_to.line_ids[0].analytic_segment_one_id.id,
            self.due_to.line_ids[1].analytic_segment_one_id.id,
            self.seg_one_a.id,
        )

        self.assertEqual(
            self.due_to.line_ids[0].analytic_segment_two_id.id,
        ...
        self.assertEqual(
            self.due_from.line_ids[0].analytic_segment_one_id.id,
            self.due_from.line_ids[1].analytic_segment_one_id.id,
            self.seg_one_a.id,
        )

        self.assertEqual(
            self.due_from.line_ids[0].analytic_segment_two_id.id,
