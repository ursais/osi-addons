...
class SaleEstimateJob(models.Model):
    ...
    description = fields.Text(string="Description of Work")
    ...
    total = fields.Float(
        compute="_compute_totalestimate", string="Total Material Estimate", store=True
    )
    ...
    @api.depends("total", "labour_total", "overhead_total")
    ...
    labour_total = fields.Float(
        compute="_compute_labour_total", string="Total Labour Estimate", store=True
    )
    overhead_total = fields.Float(
        compute="_compute_overhead_total", string="Total Overhead Estimate", store=True
    )
    ...
    def action_estimate_send(self):
        if (
            self.state == "sent"
            or self.state == "approve"
            or self.state == "quotesend"
            ...
            try:
                ...
                template_id = self.env["ir.model.data"]._xmlid_to_res_id(
                    "job_cost_estimate_customer.email_template_estimate_job",
            ...
            try:
                ...
                compose_form_id = self.env["ir.model.data"]._xmlid_to_res_id(
    ...
    def estimate_to_quotation(self):
        ...
        for rec in self:
            ...
            vals = so_id._convert_to_write({name: so_id[name] for name in so_id._cache})
            vals.update(
                {
