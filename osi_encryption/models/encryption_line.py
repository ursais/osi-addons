...
class EncryptLine(models.Model):
    ...
    def line_action_onchange(self):
        for rec in self:
            ...
            if rec.line_action == "encrypt_decrypt" and rec.state in (
                "sanitized",
                "in_progress",
            ):
                raise UserError(
                    "Can't change action to Encrypt/Decrypt because the state doesn't allow it."
                )
            if rec.line_action == "encrypt_decrypt" and rec.state in (
                "encrypted",
                "in_progress",
        ...
        if self.included_char_column_ids != self._origin.included_char_column_ids:
            ...
            for item in self._origin.included_char_column_ids.ids:
                if item not in self.included_char_column_ids.ids:
                    lines.append(item)
            move_lines_to_exclude = self.env["ir.model.fields"].search(
                [("id", "in", lines)]
            )
    ...
    def unlink(self):
        ...
        for line in self:
            if line.state == "encrypted":
                raise UserError(
                    _(
                        "You cannot delete a line that has been encrypted. Please decrypt first then delete."
                    )
                )
        return super().unlink()
