...
class TestHelpdeskTicket(TransactionCase):
    ...
    def test_helpdesk_ticket_change_stage(self):
        self.res_partner_id = self.res_partner_model.create(
            {"name": "Test Azure Interior", "company_type": "person", "can_call": True}
        )

        self.helpdesk_stage_model_id = self.helpdesk_stage_model.create({"name": "New"})
        ...
        self.backend_voicent_id = self.backend_voicent_model.create(
            {
                ...
                "call_line_ids": [
                    (
                        ...
                        0,
                        {
                            "name": "call 1",
                            "applies_on": False,
                            "helpdesk_ticket_stage_id": self.helpdesk_stage_in_progress_id.id,
                            "msgtype": "tts",
                            "msginfo": "Hello World",
                        },
                    )
                ...
                "time_line_ids": [
                    ...
                    (0, 0, {"name": "Call Time 4", "time": 16.46}),
                ],
            }
        )
