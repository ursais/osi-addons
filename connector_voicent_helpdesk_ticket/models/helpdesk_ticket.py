...
class HelpdeskTicket(models.Model):
    ...
    def voicent_check_status(self, campaign, call_line):
        for rec in self:
            ...
            message = _(
                """Status of campaign <b>%s</b> on <b>%s</b>:
             <b>%s</b>"""
                % (campaign, call_line.backend_id.name, res.get("status"))
            )
        ...
        for rec in self:
            if call_line.helpdesk_ticket_stage_id.id == rec.stage_id.id:
                ...
                res = voicent_obj.importAndRunCampaign(
                    file_name, call_line.msgtype, call_line.msginfo
                )
                ...
                if res.get("camp_id") and res.get("status") == "OK":
                    ...
                    message = _(
                        """Call has been sent to <b>%s</b> but failed
    ...
    def write(self, vals):
        for rec in self:
            if (
                vals.get("stage_id")
                and rec.partner_id.company_type
                and rec.partner_id.can_call
            ):
                ...
                for line_rec in call_lines:
                    if not (line_rec.has_parent is True and rec.parent_id is False):
                        rec.with_delay(
