...
class Test3WayMatch(common.TransactionCase):
    ...
    def test_purchase_order_with_price_difference(self):
        ...
        move_line = vendor_bill.mapped("line_ids").filtered(lambda l : l.purchase_line_id)
        move_line.price_unit = 100
        self.assertEqual(
            move_line.po_line_price_difference, True
        )
        self.assertEqual(
            move_line.move_id.po_price_difference, True
        )
    ...
    def test_purchase_order_with_no_price_difference(self):
        ...
        move_line = vendor_bill.mapped("line_ids").filtered(lambda l : l.purchase_line_id)
        self.assertEqual(
            move_line.po_line_price_difference, False
        )
        self.assertEqual(
            move_line.move_id.po_price_difference, False
        )
