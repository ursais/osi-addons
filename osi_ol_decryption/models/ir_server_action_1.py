...
class IrActionsServer(models.Model):
    ...
    def update_product_tax_code(self):
        conn_13 = psycopg2.connect(
            database="odoo13_pro_2025_01_28",
            user="odoo",
            password="odoo",
            host="localhost",
            port="5432",
    ...
    def odoo_rpc_call_product_weight(self):
        ...
        odoo_13.login("odoo13_pro_2025_01_28", "admin", "pw")
    ...
    def odoo_rpc_call_lifecycle_status(self):
        odoo_13 = odoorpc.ODOO("localhost", port=8069, timeout=12000)
        odoo_13.login("odoo13_pro_2025_01_28", "admin", "pw")
        ...
        offset = 0
        not_found_names = []
        self.env.cr.execute("UPDATE res_partner SET account_manager_id = account_manager WHERE account_manager IS NOT NULL;")
        self.env.cr.execute("UPDATE sale_order SET account_manager_id = account_manager WHERE account_manager IS NOT NULL; ")
        while True:
            product_ids = obj_product.search_read(
                [("active", "=", True)],
                fields=["lifecycle_status","pim_category"],
                order="id",
                offset=offset,
                limit=limit
            )
            ...
            for product in product_ids:
                code = product.get("lifecycle_status",False)
                if product.get("lifecycle_status",False) in ['coming_soon','in_development']:
                    code = "draft"
                if product.get("lifecycle_status",False) == 'available':
                    code = "active"
                if product.get("lifecycle_status",False) == 'preorder':
                    code = "pre_order"
                if product.get("lifecycle_status",False) == False:
                    code = "new"
                product_state_id = self.env["product.state"].search([("code","ilike",code)])
                code = product.get("pim_category",False)
                attribute_set_id = self.env["attribute.set"].search([("code","ilike",code)])
                if product_state_id:
                    self._cr.execute(
                        "update product_template set product_state_id = %s where id = %s"
                        % (product_state_id.id, product.get("id"))
                    )
                elif attribute_set_id and product.get("pim_category",False):
                    self._cr.execute(
                        "update product_template set attribute_set_id = %s where id = %s"
                        % (attribute_set_id.id, product.get("id"))
                    )
                
                elif product.get("lifecycle_status") not in not_found_names:
                    not_found_names.append(product.get("lifecycle_status"))
