    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        if self.env.context.get("wizard_id"):
            ...
            if self._context.get("field_name").startswith(qty_field_prefix):
                attr_qty_line = self._context.get("field_name").split(qty_field_prefix)[
                    1
                ]
                qty_attr_id = attr_qty_line.split("_")[1]
                attr_line_id = attr_qty_line.split("_")[0]
                local_values_dict = (
                    wiz_id.values_dict and ast.literal_eval(wiz_id.values_dict) or {}
                )
                print(
                    local_values_dict,
                    "##########>>>>>",
                ...
                for default_val in wiz_id.config_session_id.default_qty_ids:
                    template_line = PTAL.browse(int(attr_line_id))
                    qty_attr_ids = self.search(
                        [
                            ("product_attribute_id", "=", default_val.attribute_id.id),
                            (
                                "product_tmpl_id",
                                "=",
                                wiz_id.config_session_id.product_tmpl_id.id,
                            ),
                            ("product_attribute_value_id", "=", default_val.id),
                        ]
                    )
                    qty_field_name = (
                        qty_field_prefix
                        + str(attr_line_id)
                        + "_"
                        + str(default_val.attribute_id.id)
                    )
                    print(
                        "?///////", default_val, template_line, template_line.value_ids
                    )
                    if default_val.id in template_line.value_ids.ids:
                ...
                if self._context.get("field_name") and values_dict.get(
                    self._context.get("field_name")
                ):
                    ...
                    if (
                        values_dict.get("preset_product", False)
                        and value_ids is not None
                    ):
                        ...
                        domain = [
                            ...
                            (
                                ...
                                "in",
                                avq_data.mapped("template_attri_value_id").ids,
                            ),
                        ]
                        print("@#####@@@@@@@@@@@@@@", domain)
                args = domain
