...
class ConfigurationQtyCreate(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        ...
        cls.value_ssd256 = cls.env["product.attribute.value"].create(
            {"name": "SSD-256GB", "attribute_id": cls.attr_ssd.id}
        )
        ...
        cls.value_ram8gb = cls.env["product.attribute.value"].create(
            {"name": "RAM-8GB", "attribute_id": cls.attr_ram.id}
        )
        ...
        cls.attr_i7_value = cls.env["product.attribute.value"].create(
            {
                "name": "i7",
                ...
                "attribute_line_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            "attribute_id": cls.attr_processor.id,
                            "value_ids": [
                                (6, 0, cls.attr_processor.value_ids.ids),
                            ],
                            "required": True,
                            "is_qty_required": True,
                        },
        ...
        cls.ssd_config_set = cls.env["mrp.bom.line.configuration.set"].create(
            {
                "name": "SSD-Config Set",
                "configuration_ids": [
                    (0, 0, {"value_ids": [(6, 0, cls.value_ssd256.ids)]})
                ],
            }
        )

    def test_01_check_default_qty_maximum_qty(self):
        attr_ssd_val = self.env["product.template.attribute.value"].search(
            [("attribute_id", "=", self.attr_ssd.id)]
        )
        with self.assertRaises(ValidationError):
            attr_ssd_val.write({"default_qty": 2, "maximum_qty": 1})
        attr_ssd_val.write(
            {
                "default_qty": 1,
                "maximum_qty": 5,
        ...
        product_config_wizard.write(
            {
                ...
                f"__attribute_{self.attr_processor.id}": self.attr_i7_value.id,
                f"__qty_{self.attr_processor.id}": 2,
            }
        )
        product_config_wizard.action_next_step()

        # For Duplicate Product Variant Createion
        product_config_wizard2 = ProductConfWizard.create(
            {
                "product_tmpl_id": self.test_product_tmpl.id,
            }
        )
        product_config_wizard2.action_next_step()
        product_config_wizard2.write(
            {
                f"__attribute_{self.attr_ssd.id}": self.value_ssd256.id,
                f"__qty_{self.attr_ssd.id}": 2,
                f"__attribute_{self.attr_ram.id}": self.value_ram8gb.id,
    ...
    def test_03_create_mrp_order(self):
        ...
        self.value_ssd256.write(
            {"product_id": self.component_ssd.product_variant_id.id}
        )
        computer_system_bom = MRPBom.create(
            {
                ...
                "bom_line_ids": [
                    (
                        ...
                        0,
                        {
                            "product_id": self.component_ram.product_variant_id.id,
                            "product_tmpl_id": self.component_ram.id,
                            "product_qty": 2,
                            "product_uom_id": self.env.ref("uom.product_uom_unit").id,
                            "config_set_id": self.ssd_config_set.id,
                        },
                    ),
                    (
                        0,
                        ...
                        {
                            "product_id": self.component_ssd.product_variant_id.id,
                            "product_tmpl_id": self.component_ssd.id,
                            "product_qty": 1,
                            "product_uom_id": self.env.ref("uom.product_uom_unit").id,
                        },
                    ),
        ...
        self.test_product_tmpl.product_variant_id.mapped(
