        ...
        for rec in self:
            if rec.reversed_entry_id:
                payment_ids = self.env["account.payment"].search(
                    [("reversal_move_id", "=", rec.id)]
                )
                for payment_id in payment_ids:
                    partial_reconciled_line_ids = self.env[
                        ...
                        [
                            ...
                            (
                                "id",
                                "in",
                                payment_id.reconciled_bill_ids.line_ids.matched_debit_ids.ids,
                            ),
                            (
                                "id",
                                "in",
                                payment_id.reconciled_bill_ids.line_ids.matched_credit_ids.ids,
                            ),
                        ]
                    )
