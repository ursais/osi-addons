...
class AccountBankReconciliationReport(models.AbstractModel):
    ...
    def _add_bank_statement(self, statement_id):
        ...
        return {
            ...
            'level': 3,
            "class": "o_account_reports_level4",
            "unfolded": True,
            'unfoldable': True,
        }
        
    def _add_bank_statement_line(self, line, amount, parent_id=None):
        name = line.name
        line_currency = self.env.context.get("line_currency", False)
        return {
            "id": str(line.id),
            ...
            lines.append(self.
                         _add_title_line(options,
                                         _("Uneconciled Bank \
                                           Statement Lines"),
                                         level=2))
            statements_lines = self.env["account.bank.statement.line"]
            if report_data.get('not_reconciled_st_positive'):
                statements_lines |= report_data.get('not_reconciled_st_positive')
            if report_data.get('not_reconciled_st_negative'):
                statements_lines |= report_data.get('not_reconciled_st_negative')
            for bank_statement in statements_lines.mapped('statement_id'):
                lines.append(self._add_bank_statement(bank_statement))
                for line in report_data.get('not_reconciled_st_positive', []).filtered(lambda l:l.statement_id == bank_statement):
                    lines.append(self._add_bank_statement_line(line, line.amount, parent_id=bank_statement.id))
                for line in report_data.get('not_reconciled_st_negative', []).filtered(lambda l:l.statement_id == bank_statement):
                    lines.append(self._add_bank_statement_line(line, line.amount, parent_id=bank_statement.id))
        ...
        if report_data.get('reconciled_st_positive') or \
            ...
            lines.append(self.
                         _add_title_line(options,
                                         ...
                                         level=2))
            statements_lines = self.env["account.bank.statement.line"]
            if report_data.get('reconciled_st_positive'):
                statements_lines |= report_data.get('reconciled_st_positive')
            if report_data.get('reconciled_st_negative'):
                statements_lines |= report_data.get('reconciled_st_negative')
            for bank_statement in statements_lines.mapped('statement_id'):
                lines.append(self._add_bank_statement(bank_statement))
                for line in report_data.get('reconciled_st_positive', []).filtered(lambda l:l.statement_id == bank_statement):
                    lines.append(self._add_bank_statement_line(line, line.amount,parent_id=bank_statement.id))
    
                for line in report_data.get('reconciled_st_negative', []).filtered(lambda l:l.statement_id == bank_statement):
                    lines.append(self._add_bank_statement_line(line, line.amount,parent_id=bank_statement.id))
