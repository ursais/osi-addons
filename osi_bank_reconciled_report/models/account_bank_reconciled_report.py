...
class account_bank_reconciliation_report(models.AbstractModel):
    ...
    def _get_bank_rec_report_data(self, options, line_id=None):
        ...
        aml_domain = [
                     # '|', '&', ('move_id.journal_id.type', 'in', ('cash', 'bank')), ('move_id.journal_id', '=', journal_id),
        ...
        rslt['reconciled_st_positive'] = self.env['account.bank.statement.line'].search([('statement_id.journal_id', '=', journal_id),
                                                                             ('date', '<=', self.env.context['date_to']),
                                                                             ('journal_entry_ids', '!=', False),
                                                                             ('amount', '>', 0),
                                                                             ('company_id', 'in', self.env.context['company_ids'])])

        rslt['reconciled_st_negative'] = self.env['account.bank.statement.line'].search([('statement_id.journal_id', '=', journal_id),
                                                                             ('date', '<=', self.env.context['date_to']),
                                                                             ('journal_entry_ids', '!=', False),
                                                                             ('amount', '<', 0),
                                                                             ('company_id', 'in', self.env.context['company_ids'])])
    ...
    def _get_lines(self, options, line_id=None):
        ...
        # Compute totals
        unrec_tot = sum([-(self._get_amount(aml, report_data)) for aml in report_data.get('reconciled_pmts', [])])
        outstanding_plus_tot = sum([st_line.amount for st_line in report_data.get('reconciled_st_positive', [])])
        outstanding_minus_tot = sum([st_line.amount for st_line in report_data.get('reconciled_st_negative', [])])
        unrec_tot2 = sum([-(self._get_amount(aml, report_data)) for aml in report_data.get('not_reconciled_pmts', [])])
        outstanding_plus_tot2 = sum([st_line.amount for st_line in report_data.get('not_reconciled_st_positive', [])])
        outstanding_minus_tot2 = sum([st_line.amount for st_line in report_data.get('not_reconciled_st_negative', [])])
