...
class AccountBankReconciliationReport(models.AbstractModel):
    ...
    def _add_title_line(self, options, title,
                        amount=None, level=0, date=False):
        self.line_number += 1
        ...
        return {
            ...
            'name': title,
            'columns': [
                {'name': date and format_date(self.env, date) or '',
                 'class': 'date'},
            ...
            for entry_id in line.journal_entry_ids:
                if entry_id.payment_id:
                    return {
                        ...
                        'name': len(name) >= 75 and
                        name[0:70] + '...' or name,
                        'columns': [
                            {'name': format_date(self.env, line.date),
                             'class': 'date'},
                            {'name': self.format_value(amount,
                                                       line_currency)},
                            {'name': line.journal_entry_ids[0].payment_id.
                             communication},
                            {'name': line.ref},
                        ],
    ...
    def _get_amount(self, line, report_data):
        if report_data['use_foreign_currency']:
            if line.currency_id == report_data['line_currency']:
                ...
                return line.currency_id._convert(line.amount_currency,
