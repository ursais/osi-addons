...
class InventoryValuationCategory(models.AbstractModel):
    _name = "report.osi_inv_fifo.inventory_valuation_ondate_report"
    ...
    def _get_valuation_data(self, data, company_id):
        ...
        self._cr.execute(
            """
				...
				abs(price_unit) as cost
				FROM
				((
				SELECT
					pp.id as id, l.complete_name as location, pc.name as category, pt.name as product, acc1.code as code1, acc2.code as code2, pp.default_code as sku, pp.barcode as barcode, m.date,sm.price_unit as price_unit,
					coalesce(sum(-abs(m.qty_done*uom2.factor/uom.factor))::decimal, 0.0) AS qty,
					coalesce(sum(-abs(m.qty_done*uom2.factor/uom.factor) * cost.value_float)::decimal, 0.0) AS std_value,
					coalesce(sum(-abs(m.qty_done*uom2.factor/uom.factor) * abs(sm.price_unit))::decimal, 0.0) AS value
				FROM product_product pp
				LEFT JOIN stock_move_line m ON (m.product_id=pp.id)
				LEFT JOIN stock_move sm on (m.move_id=sm.id)
				...
				GROUP BY  pp.id, l.complete_name, pc.name,pt.name,acc1.code,acc2.code, pp.default_code,m.date,uom.factor,uom2.factor, sm.price_unit
				)
				UNION ALL
				(
				SELECT
				...
				(
				SELECT
					pp.id, l.complete_name as location, pc.name as pc_name, pt.name as product, acc1.code as code1, acc2.code as code2, pp.default_code,pp.barcode as barcode,m.date, sm.price_unit as price_unit,
					coalesce(sum(abs(m.qty_done*uom2.factor/uom.factor))::decimal, 0.0) AS qty,
					coalesce(sum(abs(m.qty_done*uom2.factor/uom.factor) * cost.value_float)::decimal, 0.0) AS std_value,
					coalesce(sum(abs(m.qty_done*uom2.factor/uom.factor) * abs(sm.price_unit))::decimal, 0.0) AS value
				FROM product_product pp
				...
				AS foo
				WHERE qty !=  0.0
				GROUP BY id,location,category,product,barcode,sku,account,abs(price_unit);            
            """,
            (
                start_date,
                valuation_date,
