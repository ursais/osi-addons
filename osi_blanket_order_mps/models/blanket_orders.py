...
class BlanketOrder(models.Model):
    ...
    def action_mps_replenish(self, line_ids):
        ...
        for line in line_ids:
            ...
            if mps_id:
                existing_forecast = mps_id.forecast_ids.filtered(
                    lambda l: l.date == line.date_schedule
                )
                if existing_forecast:
                    qty = 0
                    if existing_forecast.replenish_qty:
                        blanket_line_ids = sale_blanket_line_obj.search(
                            [
                                ("product_id", "=", line.product_id.id),
                        ...
                        qty = sum(blanket_line_ids.mapped("remaining_uom_qty"))
                    qty = qty + line.remaining_uom_qty
                    existing_forecast.write({"forecast_qty": qty, "replenish_qty": qty})
                    ...
                    mrp_ids.unlink()
                    mps_id.with_context(
                        mps_date=line.date_schedule,
                        mps_qty=line.remaining_uom_qty,
                        mps_id=mps_id.id,
                        product_id=line.product_id.id,
                    ).action_replenish()
                else:
                    ...
                    qty = sum(blanket_line_ids.mapped("remaining_qty"))
                    qty = qty + line.remaining_qty
                    existing_forecast.create(
                        {
                            "forecast_qty": qty,
                            "date": line.date_schedule,
                            "replenish_qty": qty,
