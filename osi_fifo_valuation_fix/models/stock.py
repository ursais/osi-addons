    ...
    def _run_fifo(self, move, quantity=None):
        ...
        if move.product_id.tracking == "none":
            ...
            for valued_move_line in valued_move_lines:
                ...
                # Find back incoming stock moves (candidates) to value this move.
                qty_to_take_on_candidates = quantity or valued_quantity
                for candidate in candidates:
                    # Check if the lot_id of the move line matches
                    # the lot_id for the candidate lines.
                    allowed_canditates_lines = self.env["stock.move.line"].search(
                        [
            ...
            # at the price of the last out and a correction entry will be made once
            # `_fifo_vacuum` is called.
            if qty_to_take_on_candidates == 0:
                # If the move is not valued yet we compute the price_unit based
                # on the value taken on the candidates.
                # If the move has already been valued, it means that we editing
                # the qty_done on the move. In this case, the price_unit computation
                # should take into account the quantity already valued and the new
                # quantity taken.
                if not move.value:
                    price_unit = -tmp_value / (move.product_qty or quantity)
                else:
                    price_unit = (
                        -(tmp_value) + move.value) / (tmp_qty + move.product_qty)
                move.write({
