...
class TestBomPrice(TestSaleTierValidation):
    ...
    def test_01_bom_report(self):
        ...
        # 5 min 'Prepare biscuits' + 3 min 'Prepare butter' + 5 min 'Mix manually' =
        # 13 minutes for 1 biscuits so 13 * 11 = 143 minutes
        self.assertEqual(
        ...
        for component_line in report_values["lines"]["components"]:
            # standard price * bom line quantity * current quantity / bom finished
            # product quantity
        ...
        for component_line in report_values["lines"]["components"]:
            ...
            # product quantity
            if component_line["product"].id == butter.id:
                # 5 kg of butter at 7.01$ for 11kg of crumble -> / 11 for
                # price per unit (3.19)
                self.assertEqual(
                    float_compare(
            ...
            if component_line["product"].id == biscuit.id:
                # 6 kg of biscuits at 1.50$ for 11kg of crumble -> / 11 for
                # price per unit (0.82)
                self.assertEqual(
                    float_compare(
                        component_line["bom_cost"],
        ...
        # Operation cost is the sum of operation line : (60 * 10)/60 * 10€ +
        # (10 + 15 + 60 * 5)/60 * 20€ + (1 + 2)/60 * 20€ = 209,33€
        self.assertEqual(
            float_compare(
        ...
        # total price = Cream (15.51€) + crumble_cost (34.63 €) +
        # operation_cost(209,33) = 259.47€
        self.assertEqual(
            float_compare(
