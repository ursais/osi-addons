...
class ReportBomStructure(models.AbstractModel):
    ...
    def _get_bom_data(self, bom, warehouse, product=False, line_qty=False, bom_line=False, level=0, parent_bom=False, parent_product=False, index=0, product_info=False, ignore_stock=False):
        bom_report_line = super()._get_bom_data(bom, warehouse, product=product, line_qty=line_qty, bom_line=bom_line, level=level, parent_bom=parent_bom, parent_product=parent_product, index=index, product_info=product_info, ignore_stock=ignore_stock)
        ...
        if not is_minimized:
            if product:
                prod_price = product.uom_id._compute_price(product.with_company(company).lst_price, bom.product_uom_id) * current_quantity
            else:
                # Use the product template instead of the variant
                prod_price = bom.product_tmpl_id.uom_id._compute_price(bom.product_tmpl_id.with_company(company).lst_price, bom.product_uom_id) * current_quantity
    ...
    def _get_component_data(self, parent_bom, parent_product, warehouse, bom_line, line_quantity, level, index, product_info, ignore_stock=False):
        component_line = super()._get_component_data(parent_bom, parent_product, warehouse, bom_line, line_quantity, level, index, product_info, ignore_stock=ignore_stock)
        company = parent_bom.company_id or self.env.company
        price = bom_line.product_id.uom_id._compute_price(bom_line.product_id.with_company(company).lst_price, bom_line.product_uom_id) * line_quantity
    ...
    def _get_bom_array_lines(self, data, level, unfolded_ids, unfolded, parent_unfolded=True):
        ...
        for bom_line in bom_lines:
            ...
            lines.append({
                ...
                'prod_price': bom_line['prod_price'] if bom_line.get('prod_price') else 0,
